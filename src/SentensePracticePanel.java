    ...
    public SentensePracticePanel(MainFrame mf, String language){
        ...
        try{
            ...
            for(int i=0; i<size;){
                ...
                if(text.contains(tmp.get(ran))) continue; // 이미 포함된 문장일 경우 continue
                if(tmp.get(ran).trim().equals("")) continue; // 빈 문장일 경우 continue
                if(tmp.get(ran).trim().length()==1)continue; // 글자수가 1자 일 경우 continue
                else { // 아닐 경우
                    size--; // 입력해야할 문장 수 감소
    ...
    class TypeRunnable implements Runnable{
        public void run(){ //스레드 실행 함수
           while(true){
                ...
                if(n<0) n=0; // n이 0 미만일 경우 n 초기화
                if(n>1000) n = 1000; // n이 1000 초과할 경우 n 초기화
        ...
        public void keyTyped(KeyEvent e){
            ...
            if(key== KeyEvent.VK_BACK_SPACE){ // 백스페이스 입력 시 실행
                ...
                if(typeLabel.getText().length()>0){ // 입력된 텍스트가 있을 경우
                    ch =  questionLabel[0].getText().charAt(typeLabel.getText().length()-1); // 현재 입력한 문장 길이와 같은 인덱스의 문제 문자를 가져옴
                    if(typeLabel.getText().charAt(typeLabel.getText().length()-1) != ch) { // 현재 입력한 마지막 문자와 ch가 다를 경우
                        inaccuracy_cnt--; // 오타 수 증가
                        accuracy = 100 - (int)((double)inaccuracy_cnt/typeLabel.getText().length() * 100); //정확도 재계산
                ...
                if(Integer.parseInt(highestType_Label.getText())<Integer.parseInt(currentType_Label.getText())){ // 현재타수가 최고타수보다 높을 경우
                ...
                if(questionLabel[0].getText().length() == typeLabel.getText().length()){  // 문제 문장 텍스트 길이와 입력창 텍스트 길이가 같을 경우
                    ...
                    typo++; // 타수 증가
                    else inaccuracy_cnt++; // 아닐 경우 오타수 증가
    ...
    class BackToMainEvent extends KeyAdapter{ // 패널 키 이벤트
        public void keyTyped(KeyEvent e){ // 키 입력했을 경우
            if(e.getKeyChar() == KeyEvent.VK_ESCAPE){ // 'Esc' 키일 경우
                typeLabel.setText(typeLabel.getText().substring(0, typeLabel.getText().length()-1)); // 입력창에 'Esc' 키 입력 된 것을 지워줌
                th.interrupt(); // 스레드 강제 종료

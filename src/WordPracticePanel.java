...
public class WordPracticePanel extends JPanel {
    ...
    private final char key_lay4[] = { '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '-', '=' }; // 키 매핑을 위한 배열
    private final char key_lay4_2[] = { '!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '_', '+' }; // 키 매핑을 위한 배열
    private final char key_lay5[] = { ';', '\'' }; // 키 매핑을 위한 배열
    private final char key_lay5_2[] = { ':', '\"' }; // 키 매핑을 위한 배열
    private final char key_lay6[] = { ',', '.', '/' }; // 키 매핑을 위한 배열
    private final char key_lay6_2[] = { '<', '>', '?' }; // 키 매핑을 위한 배열
    private final char key_lay7[] = { '[', ']' }; // 키 매핑을 위한 배열
    private final char key_lay7_2[] = { '{', '}' }; // 키 매핑을 위한 배열
    ...
    private JLabel spaceImage; // Space 키 매핑 이미지
    private JLabel enterImage; // enter 키 매핑 이미지
    private char ch;
    ...
    public WordPracticePanel(MainFrame mf, String language) {
        ...
        spaceImage = new JLabel(new ImageIcon("img/keyPress.png")); // Space 매핑 이미지 생성 및 설정
        spaceImage.setSize(25, 25);
        spaceImage.setLocation(555, 600);
        spaceImage.setVisible(false);
        add(spaceImage);

        enterImage = new JLabel(new ImageIcon("img/keyPress.png")); // Enter 매핑 이미지 생성 및 설정
        enterImage.setSize(25, 25);
        enterImage.setLocation(1070, 440);
    ...
    public void KeyImageChange(char ch) { // 키 매핑을 위한 함수
        ...
        for (int i = 0; i < key_lay4.length; i++) {
            if (ch == key_lay4[i]) { // 문자가 key_lay4에 있을 경우
                ...
                return; // 종료
            } else if (ch == key_lay4_2[i]) { // 문자가 key_lay4에 있을 경우
                shiftImage.setVisible(true); // 'A' ~ 'Z' 가 입력 되었을 경우 shiftImage 보이게 설정
                charImage.setLocation(180 + (i * 70), 340); // charImage 이미지 위치 변경
                return; // 종료
            }
        }

        for (int i = 0; i < key_lay5.length; i++) {
            if (ch == key_lay5[i]) { // 문자가 key_lay4에 있을 경우
                charImage.setLocation(855 + (i * 70), 470); // charImage 이미지 위치 변경
                return; // 종료
            } else if (ch == key_lay5_2[i]) { // 문자가 key_lay4에 있을 경우
                shiftImage.setVisible(true); // ':' , '"' 가 입력 되었을 경우 shiftImage 보이게 설정
                charImage.setLocation(855 + (i * 70), 340); // charImage 이미지 위치 변경
                return; // 종료
            }
        }

        for (int i = 0; i < key_lay6.length; i++) {
            if (ch == key_lay6[i]) { // 문자가 key_lay4에 있을 경우
                charImage.setLocation(760 + (i * 70), 535); // charImage 이미지 위치 변경
                return; // 종료
            } else if (ch == key_lay6_2[i]) { // 문자가 key_lay4에 있을 경우
                shiftImage.setVisible(true); // '<', '>', '?' 가 입력 되었을 경우 shiftImage 보이게 설정
                charImage.setLocation(760 + (i * 70), 535); // charImage 이미지 위치 변경
                return; // 종료
            }
        ...
        for (int i = 0; i < key_lay7.length; i++) {
            if (ch == key_lay7[i]) { // 문자가 key_lay4에 있을 경우
                charImage.setLocation(917 + (i * 70), 400); // charImage 이미지 위치 변경
                return; // 종료
            } else if (ch == key_lay7_2[i]) { // 문자가 key_lay4에 있을 경우
                shiftImage.setVisible(true); // '{' , '}' 가 입력 되었을 경우 shiftImage 보이게 설정
                charImage.setLocation(917 + (i * 70), 400); // charImage 이미지 위치 변경
                return; // 종료
            }
        ...
        if (ch + 32 >= 'a' && ch + 32 <= 'z')
            ...
            shiftImage.setVisible(false); // 아닐 경우 shiftImage 안보이게 설정
    ...
    class TypeEvent extends KeyAdapter {
        public void keyTyped(KeyEvent e) {
            ...
            if (key == KeyEvent.VK_BACK_SPACE) { // 백스페이스 입력 시 실행
                ...
                if (questionLabel.getText().substring(0, typeLabel.getText().length()).equals(typeLabel.getText())) // 한문자씩
                                                                                                                    // 올바르게
                                                                                                                    // 입력했는지
                ...
                if (progress == 100) { // 모두 끝 마쳤을 경우
                    ...
                    if (index == max - 1)
                        questionLabel2.setText(""); // 마지막 문제일 경우 다음 문제 단어 표시 안함
                    else
                        questionLabel2.setText(text.get(index + 1)); // 마지막 문제가 아닐 경우 다음 문제 단어 표시
                    typeLabel.setText(""); // 재입력 받을 준비함.
                    KeyImageChange(questionLabel.getText().charAt(0)); // 다음 문제 입력 첫 문자를 키보드 하이라이트에 표시
                }
            } else {
                if (questionLabel.getText().length() == typeLabel.getText().length()) { // 문제 단어 텍스트 길이와 입력창 텍스트 길이가 같을
                                                                                        // 경우

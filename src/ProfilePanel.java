...
public class ProfilePanel extends JPanel{
    ...
    public ProfilePanel(MainFrame mf){
        ...
        settingLabel[1].setText(Integer.toString(mf.getGoalType())); // MainFrame에 있는 getGoalType 함수를 사용하여 목표 타수를 가져와 Label Text로 설정
        settingLabel[2].setText(Integer.toString(mf.getGoalAccuracyValue())); // MainFrame에 있는 getGoalAccuracyValue 함수를 사용하여 목표 정확도를 가져와 Label Text로 설정
    ...
    private int showMultipleInputMessageDialog(){ // 프로필 변경 버튼 눌렀을 시 실행할 Custom Dialog
        ...
        if(option == JOptionPane.OK_OPTION){ // OK버튼 눌렀을 경우
            ...
            if(tf2.getText() == "") goalValue = 100; // tf2가 비었을 경우 목표 타수 100 설정
            else goalValue = Integer.parseInt(tf2.getText()); // 아닐 경우 목표 타수 설정
            if(tf3.getText() == "")  accuracyValue = 90; // tf3가 비었을 경우 목표 정확도 90 설정
            else accuracyValue = Integer.parseInt(tf3.getText()); // 아닐 경우 목표 정확도 설정
        }
            ...
            if(showMultipleInputMessageDialog()==0) // CustomDialog 실행하여 확인 버튼을 눌렀을 경우
            {
                while(true){
                    if(nickName.length()>8){ // 별명이 8자리 이상일 경우
                        ...
                        if(showMultipleInputMessageDialog()!=0) break; // 취소 버튼을 눌렀을 경우 break
                    }
